...
void env_free(struct Env *e)
{
	...
	// [2] Free the PAGE working set array itself from the main memory
		void* va =e->ptr_pageWorkingSet;
		/*uint32* pt=NULL;
		struct Frame_Info* frame =get_frame_info(e->env_page_directory,va,&pt);
		free_frame(frame);*/
		kfree(va);
		// [3] Free all TABLES from the main memory
		for(uint32 i=0;i<1024;i++){
			uint32 pt=kheap_virtual_address(e->env_page_directory[i]);
			kfree(&pt);

		}
	// [4] Free the page DIRECTORY from the main memory
		uint32 aa= (uint32)e->env_page_directory;
		uint32 vv=kheap_virtual_address(aa);
			kfree(&vv);

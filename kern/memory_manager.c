...
void allocateMem(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	int allocatePages = size/PAGE_SIZE+(size%PAGE_SIZE!=0);
	for(int i=0;i<allocatePages;i++){
...
void freeMem(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	int allocateFrames = (size/PAGE_SIZE)+(size%PAGE_SIZE);
	int lva=virtual_address+(allocateFrames*PAGE_SIZE);
	uint32 va = virtual_address;
	for(int i=0;i<allocateFrames;i++){
		pf_remove_env_page(e,va); va+=PAGE_SIZE;
	}


	for(int i=0;i<e->page_WS_max_size;i++){
		if(!e->ptr_pageWorkingSet[i].empty&&(e->ptr_pageWorkingSet[i].virtual_address>=virtual_address)
				&&(e->ptr_pageWorkingSet[i].virtual_address<lva)){
			uint32 va=e->ptr_pageWorkingSet[i].virtual_address;
			uint32* pt;
			struct Frame_Info* frame = get_frame_info(e->env_page_directory,
					(void*)va,&pt);
			free_frame(frame);

...
void remove_percentage(struct Env *curenv,uint32 removePagesCount){
	for(int i=0;i<removePagesCount;i++){
			uint32 minn=0xFFFFFFFF,minIndex=0;
			for(int j=0;j<curenv->page_WS_max_size;j++){
				if(!curenv->ptr_pageWorkingSet[j].empty&&
						curenv->ptr_pageWorkingSet[j].time_stamp<minn){
					minIndex=j;
					minn=curenv->ptr_pageWorkingSet[j].time_stamp;
				}

			}
	uint32* pt=NULL;
	struct Frame_Info* frame=get_frame_info
						(curenv->env_page_directory,(void*)curenv->ptr_pageWorkingSet[minIndex].virtual_address,&pt);
						uint32 perm = pt_get_page_permissions(curenv,curenv->ptr_pageWorkingSet[minIndex].virtual_address) ;
						if(perm&PERM_MODIFIED)
							pf_update_env_page(curenv,(void*)curenv->ptr_pageWorkingSet[minIndex].virtual_address,frame);
						unmap_frame(curenv->env_page_directory,(void*)curenv->ptr_pageWorkingSet[minIndex].virtual_address);
						env_page_ws_clear_entry(curenv,minIndex);
		}
...
void page_fault_handler(struct Env * curenv, uint32 fault_va){
	...
	int r=memory_scarce_threshold_percentage*heapSize/100;
	r+=(r*100!=memory_scarce_threshold_percentage*heapSize);
	if(calculate_free_frames()<r){
		uint32 removePagesCount=curenv->percentage_of_WS_pages_to_be_removed*env_page_ws_get_size(curenv)/100;
		removePagesCount+=(curenv->percentage_of_WS_pages_to_be_removed*env_page_ws_get_size(curenv)%100!=0);
		remove_percentage(curenv,removePagesCount);
		for(int i=0;i<num_of_ready_queues;i++){
			struct Env_Queue* queue=&env_ready_queues[i];
			int sz=queue_size(queue);
			for(int j=0;j<sz;j++){
				struct Env* e = dequeue(queue);
				removePagesCount=e->percentage_of_WS_pages_to_be_removed*env_page_ws_get_size(e)/100;
						removePagesCount+=(e->percentage_of_WS_pages_to_be_removed*env_page_ws_get_size(e)%100!=0);
				remove_percentage(e,removePagesCount);
			}
		}
		int sz=queue_size(&env_exit_queue);
		for(int i=0;i<sz;i++){
			struct Env* e = dequeue(&env_exit_queue);

			remove_percentage(e,env_page_ws_get_size(e));
		}
	}

	uint32 removePagesCount=curenv->percentage_of_WS_pages_to_be_removed*curenv->page_WS_max_size/100;
	removePagesCount+=(curenv->percentage_of_WS_pages_to_be_removed*env_page_ws_get_size(curenv)%100!=0);
	...
	struct Frame_Info* frame;
				...
				map_frame(curenv->env_page_directory,frame,(void*)fault_va,PERM_PRESENT|PERM_WRITEABLE|PERM_USER);
				...
				if(ret==E_PAGE_NOT_EXIST_IN_PF)
					{
						if(fault_va>=USER_HEAP_MAX&&fault_va<USTACKTOP)
							...
							panic("access page that doesn't exist");
					}

				env_page_ws_set_entry(curenv,q,fault_va);
				//env_page_ws_print(curenv);

...
//============================ REQUIRED FUNCTIONS ==================================//
//==================================================================================//
int UheapPages[262199]={};
uint32 nextt=USER_HEAP_START;
int Ualloc=1;
void* malloc(uint32 size)
{
	...
	uint32 s=nextt;
	bool ok=0,l=(nextt==USER_HEAP_START);
	for(uint32 i =tmp;1;i+=PAGE_SIZE){
		uint32 t = (i-USER_HEAP_START)/PAGE_SIZE;
		if(UheapPages[t]){
			tmp=s=i+PAGE_SIZE;cnt=0; continue;
		}
		cnt++;
		if(cnt==numOfFrames){
					ok=1;tmp=i+PAGE_SIZE; break;
				}
		if(i>=USER_HEAP_MAX-PAGE_SIZE&&!l)
				tmp=s=USER_HEAP_START,i=USER_HEAP_START-PAGE_SIZE,cnt=0, l=1;
		else if(i>=USER_HEAP_MAX-PAGE_SIZE&&l)break;
	}
	if(!ok){return NULL;}
	for(uint32 i=s;i<tmp;i+=PAGE_SIZE){
		int t = (i-USER_HEAP_START)/PAGE_SIZE;
		UheapPages[t]=Ualloc;
	}
	Ualloc++;
	sys_allocateMem(s,size);
	uint32* ret = (uint32*)s;
	nextt = (tmp==0xA0000000)?USER_HEAP_START:tmp;
	return ret;
}
...
void free(void* virtual_address)
{
	...
	int t=(va-USER_HEAP_START)/PAGE_SIZE;
	int u=UheapPages[t],sz=0;
	for(;(u&&UheapPages[t]==u);){
		UheapPages[t++]=0; sz++;
	}
	sys_freeMem(va,sz);
}
